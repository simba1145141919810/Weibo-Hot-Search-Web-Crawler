...
def make_bar_chart(df, current_time_str, filename_time_str):
    ...
    plt.tight_layout()
    chart_path = f"{SNAPSHOT_DIR}/当次分类柱状图_{filename_time_str}.png"
    plt.savefig(chart_path, dpi=150, transparent=False)
    plt.close()
    print(f"📊 当次柱状图已生成: {chart_path}")


def make_historical_bar_chart(current_time_str, filename_time_str):
    """【新增】图表 2：读取数据库，绘制所有历史数据的总计柱状图"""
    try:
        conn = sqlite3.connect(DB_FILE)
        ...
        plt.xlabel("AI 智能分类", fontsize=12, labelpad=10)
        plt.ylabel("历史累计登榜次数", fontsize=12, labelpad=10)
        plt.grid(axis='y', linestyle='--', alpha=0.3)
        ...
        plt.tight_layout()
        chart_path = f"{SNAPSHOT_DIR}/历史总计柱状图_{filename_time_str}.png"
        plt.savefig(chart_path, dpi=150, transparent=False)
        plt.close()
        print(f"🗃️ 历史总计柱状图已生成: {chart_path}")
    except Exception as e:
        print(f"📉 生成历史总计柱状图失败: {e}")


def make_line_chart(current_time_str, filename_time_str):
    """图表 3：读取数据库，绘制不同时段不同分类的数量曲线"""
    try:
        conn = sqlite3.connect(DB_FILE)
...
def start_one_round():
    ...
    make_historical_bar_chart(cur_time, file_time)
    make_line_chart(cur_time, file_time)
